...
def cli(ctx: click.Context, verbose: bool, version: bool) -> None:
    ...
    A powerful tool for building Blender extensions that include Python packages
    as wheels, with cross-platform support and intelligent dependency management.
    """
    setup_logging(verbose)
    
    if version:
        console.print(f"[bold blue]extbpy[/bold blue] version [bold green]{__version__}[/bold green]")
        sys.exit(0)
...
@click.option('--source-dir', '-s', type=click.Path(exists=True, path_type=Path), 
              default=Path.cwd(), help='Source directory containing extension files')
@click.option('--output-dir', '-o', type=click.Path(path_type=Path),
              default=Path.cwd(), help='Output directory for built extensions')
...
def build(
    ...
    try:
        ...
        if 'all' in platform:
            if len(platform) > 1:
                console.print("[yellow]Warning: 'all' specified with other platforms. Using all platforms.[/yellow]")
            console.print("[blue]Building for all supported platforms...[/blue]")
...
@cli.command()
@click.option('--source-dir', '-s', type=click.Path(exists=True, path_type=Path),
              default=Path.cwd(), help='Source directory to clean')
@click.option('--pattern', '-p', multiple=True, default=['.blend1', '.MNSession'],
...
@cli.command()
@click.option('--source-dir', '-s', type=click.Path(exists=True, path_type=Path),
              default=Path.cwd(), help='Source directory containing pyproject.toml')
@click.option('--platform', '-p', multiple=True,
              type=click.Choice(['windows-x64', 'linux-x64', 'macos-arm64', 'macos-x64', 'all']),
              help='Target platforms (can be specified multiple times). Use "all" for all supported platforms')
@click.option('--python-version', default='3.11',
...
def download(
    ...
    try:
        ...
        if 'all' in platform:
            if len(platform) > 1:
                console.print("[yellow]Warning: 'all' specified with other platforms. Using all platforms.[/yellow]")
            console.print("[blue]Downloading wheels for all supported platforms...[/blue]")
            platform = ['windows-x64', 'linux-x64', 'macos-arm64', 'macos-x64']
        elif not platform:
            console.print("[yellow]No platforms specified, detecting current platform...[/yellow]")
            platform = builder.detect_current_platform()
...
def info(source_dir: Path) -> None:
    ...
    try:
        ...
        console.print(f"[bold blue]📋 Project Information[/bold blue]")
